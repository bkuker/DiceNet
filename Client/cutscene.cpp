...
HRESULT PlayCutscene(LPTSTR szMovie, HINSTANCE hInstance, HWND _parent)
{
    HRESULT hr;
	parent = _parent;
...
HRESULT PlayMedia(LPTSTR lpszMovie, HINSTANCE hInstance)
{
    ...
    //wcsncpy(wFileName, (const unsigned short *)lpszMovie, MAX_PATH-1);
	for(unsigned int i = 0; i < strlen(lpszMovie); i++){
		wFileName[2*i] = lpszMovie[i];
		wFileName[2*i+1] = 0;
	}
    wFileName[MAX_PATH - 1] = 0;

    // Allow DirectShow to create the FilterGraph for this media file
    hr = pGB->RenderFile(L"intro2.avi", NULL);
    if (FAILED(hr)) {
        return hr;
    }
 ...
 /*   // Set fullscreen
	...
	hr = pVW->put_Owner((OAHWND)parent);
	if (FAILED(hr))
    {
        return hr;
    }
	...
	RECT grc;
	GetClientRect(parent, &grc);

	hr = pVW->SetWindowPosition(0, 0, grc.right, grc.bottom);
	if (FAILED(hr))
    {
        return hr;
    }
